...
class PatchPositionEmbedding(nn.Module):
    def __init__(self, dim: int, H: int, W: int):
        super().__init__()
        self.pos_embedding = nn.Parameter(torch.randn(dim, H, W)) # (dim, H, W)

    def forward(self, image):
        # image is of shape (N, C, H, W)
        # add pos embedding to image and flatten to (N, C, H*W)
        return (image + self.pos_embedding[None, :, :, :]).flatten(2)
...
class RM2(nn.Module):
    def __init__(
        ...
        self.encoder = nn.Sequential(
            ...
            PatchPositionEmbedding(encoder_dim, H//encoder_patch_size*2, W//encoder_patch_size*2),
        )
        ...
        self.encoding_proj_up = None
        self.encoding_proj_down = None
        if transformer_hidden_size != encoder_dim:
            self.encoding_proj_up = nn.Linear(encoder_dim, transformer_hidden_size)
            self.encoding_proj_down = nn.Linear(transformer_hidden_size, encoder_dim)
        self.kspace_embedding_proj_up = None
        if transformer_hidden_size != kspace_embedding_dim:
            self.kspace_embedding_proj_up = nn.Linear(kspace_embedding_dim, transformer_hidden_size)
    ...
    def forward(self, kspace_masked, col_mask):
        ...
        for _ in range(n_iters):
            ...
            image2_encoded_patch_vectors = self.encoder(image2_complex) # (N, C_enc, H/ps*W/ps)
            kspace1_encoded_column_vectors = self.kspace_embedding(kspace1_dc, col_mask) # (N, C_kspace, W)
            if self.encoding_proj_up is not None:
                image2_encoded_patch_vectors = self.encoding_proj_up(image2_encoded_patch_vectors.permute(0, 2, 1)).permute(0, 2, 1) # (N, C_trans, H/ps*W/ps)
            if self.kspace_embedding_proj_up is not None:
                kspace1_encoded_column_vectors = self.kspace_embedding_proj_up(kspace1_encoded_column_vectors.permute(0, 2, 1)).permute(0, 2, 1) # (N, C_trans, W)

            transformer_output = self.transformer(image2_encoded_patch_vectors.permute(0, 2, 1), kspace1_encoded_column_vectors.permute(0, 2, 1)).permute(0, 2, 1) # (N, C_trans, H/ps*W/ps)
            if self.encoding_proj_down is not None:
                transformer_output = self.encoding_proj_down(transformer_output.permute(0, 2, 1)).permute(0, 2, 1) # (N, C_enc, H/ps*W/ps)
            decoder_input = transformer_output.reshape(
                -1, 
                self.encoder_dim, 
                self.H//self.encoder_patch_size*2, 
                self.W//self.encoder_patch_size*2) # (N, C_enc, H/ps, W/ps)
            image3_denoised = self.decoder(decoder_input) # (N, 2, H, W)
